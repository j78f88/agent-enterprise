...
- Run `` and `` after edits — fix before committing
- Run scoped tests for touched files only (e.g., the component's test file, or `python -m pytest tests/ -v` if a store was modified). Do NOT run the full test suite — @qa handles that in Phase 3.
