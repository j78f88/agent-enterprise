...
- If you adopt an external tracker, stop maintaining a parallel markdown
  ...
  read-only mirror.
- Keep handoff state in one place — either inside the tracker issue or in a
  single manifest file per session, not both.

---

## Pattern 3 — Governance-before-proven-loop
...
question — *"can the dispatcher reliably tell whether an agent did the
work?"* — was assumed solved.
...
profile system's intent — that project-specific config lives in the consuming
project — was documented but not enforced at the repo boundary.
...
- If you need to extend or override a generic instruction, do it in your
  ...
  [docs/EXTENSION_GUIDE.md](docs/EXTENSION_GUIDE.md).
- If a change feels generic enough to upstream, open a PR — but make the
...
1. **Three repos, not one.**
   - `agent-homebase` — generic skills library (this repo).
   - `<your>-orchestration` — dispatch logic, tracker integration,
     completion-verification rules, retry/escalation policy.
   - `<your>-project` — application code plus project-specific governance
