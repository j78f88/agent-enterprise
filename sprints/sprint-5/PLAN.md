...
- [x] Create the package mirroring the `src/phase1_verification/` layout
      (`__init__.py` + focused modules).
- [x] Port the dispatcher core from
...
- [x] Port the ghost-done verifier (`dispatcher.py:135-167`): never mark
      done solely because the session ended; verifier outputs +
      return-tier validation decide.
- [x] Reference impl files are **not modified** — port by copy +
...
- [x] Discover `*.callable.yml` sidecar manifests per the non-enterprise
      example in
      `command-centre/01-protocols/callable-contract.md` (~L93-116).
- [x] Discover `callable-v1` frontmatter in skill files (the enterprise
      path), reusing the frontmatter parsing already proven in `init.py`.
- [x] Validate every discovered callable against
      `schemas/callable-v1.schema.json`; invalid callables are reported
      with path + violation, never silently skipped.
- [x] Deterministic discovery order (sorted paths) so runs are
      reproducible.
...
- [x] `queue_file.py`: atomic `state.yml` writes (write-temp + `os.replace`),
...
- [x] Honour `.mode-2-pins` (contract pin file) when loading queue
      config.
- [x] `returns.py`: validate subagent returns by **reusing**
...
- [x] No schema or contract text changes: `mode-2-contract-v1` and
      `protocol-v1` remain frozen.

### Task Group 4: Root CLI `dispatch.py`
Files: `dispatch.py` (new, repo root)

- [x] Subcommands: `run` (drain inbox, emit tier-3 summary), `status`
...
- [x] Mirror `init.py` CLI conventions (argparse, clear errors, non-zero
...
- [x] `--queue-root` argument with the same relative-path containment
...
- [x] State-machine tests: every allowed transition, every illegal
      transition raises `TransitionError`.
- [x] Ghost-done tests: session end without verifier evidence never
      yields `done`.
- [x] Crash-resume tests: kill mid-dispatch (simulated partial
      state/journal), restart, queue reconciles without loss or
      duplicate dispatch; state writes are atomic (no torn `state.yml`).
- [x] Non-enterprise callable end-to-end: a `*.callable.yml` sidecar
      (per `callable-contract.md` ~L98-116) is discovered, dispatched,
      and verified with zero enterprise-substrate coupling.
- [x] CLI tests: `validate-callables` exit codes; `requeue` rejects
      illegal transitions.
...
- [x] Extend the Mode 2 conformance hook
...
- [x] Add a byte-freeze test: hash the reference-impl files
...
- [x] Write `docs/ORCHESTRATION.md`: install, queue layout, callable
      authoring (sidecar + frontmatter), `dispatch.py` usage,
      crash-recovery semantics.
- [x] Update `command-centre/03-mode-orchestration/install-contract.md`
      to name `src/mode2_dispatcher/` + `dispatch.py` as the supported
      implementation (per ADR 0008 criterion 4), keeping the contract
      text itself frozen.
- [x] README: Mode 2 row/claims updated to "supported implementation";
      CHANGELOG entry.
...
- [x] standard — 600 passed / 11 skipped, green with -W error::DeprecationWarning; Windows leg binds at CI (2026-06-10)
- [x] Determinism — init.py double-build identical; validate-callables stdout byte-identical across runs (2026-06-10)
- [x] guardrail — check_tokens.py + check_build_command.py exit 0 (2026-06-10)
- [x] docs — 108 passed / 6 skipped on the doc subset (2026-06-10)
- [x] conformance — parametrized checklist passes for both impls; byte-freeze hashes pinned by test (2026-06-10)

## Success Criteria

- [x] `src/mode2_dispatcher/` meets all five ADR 0008 criteria (src/ + root CLI; pytest contract checklist on both OSes; crash-safety beyond reference; adopter doc + named in install-contract; reference impl frozen).
- [x] `python dispatch.py run` drains a fixture queue and emits a valid tier-3 summary; crash-resume proven by test.
- [x] A non-enterprise `*.callable.yml` callable round-trips end-to-end.
- [x] Shared conformance parametrization covers both implementations.
- [x] `docs/ORCHESTRATION.md` published; install-contract names the supported impl; README/CHANGELOG updated.
