...
**Status:** Active · **Type:** Mixed (bug-fix / debt)
...
- `init.py` — deploy-copy step, missing-key fail-on-unresolved, `.claude/commands/` seeding (TG1, TG5)
- `config/project.config.yml` + profiles — missing `commands.*` / `security:` keys (TG1), `paths.claude_commands` token (TG5)
- `scripts/` + CI config — token-free guardrail (TG2), canonical-build-command check (TG3)
- `tests/test_init.py` — tests for TG1, TG2; `SOURCE_STYLE_REFS` derivation (TG4)
- `skills/onboarding/onboarding.skill.md` — Claude Code setup section (TG5)
- `agents/planner.body.md` + `skills/planner/*` — checkpoint enforcement (TG6)
- `SPRINTS.md`, `docs/planning/BACKLOG_LEDGER.md`, `docs/planning/BUG_BACKLOG.md` — demo purge (TG7)

## Quality Gates

- [x] standard — `python -m pytest tests/ -v` stays green (401+)
...
- Purge safety (Task Group 7): grepped in-flight drafts — zero matches. No draft depends on purged demo content; purge is safe.
