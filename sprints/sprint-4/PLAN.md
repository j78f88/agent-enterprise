...
- [x] Add a `paths.claude_agents` token (default `.claude/agents`) to the
      example config and all three profiles, alongside the existing
      `paths.claude_commands` (profiles ~L52-53).
- [x] Emit one native subagent file per agent into `paths.claude_agents`,
...
- [x] Seeding is deterministic, sorted, and idempotent (mirror the
      `.claude/commands/` seeding pattern at `init.py:850-860`).
- [x] Respect the absolute-path deploy guard (`init.py:805-816`) for the
      new token.
...
- [x] Add a `paths.cursor_commands` token (default `.cursor/commands`)
      to the example config and all three profiles.
- [x] Mirror the Claude commands seeding logic (`init.py:850-860`):
...
- [x] Include the new directory in the post-deploy token-free scan
...
- [x] Implement `emit_codex_agents_md()`: render the agent roster +
      pointers to deployed skills into a managed block delimited by
      `<!-- agent-enterprise:begin -->` / `<!-- agent-enterprise:end -->`.
- [x] Merge semantics: if the adopter's `AGENTS.md` exists, replace only
...
- [x] Dogfood: run the `codex` emission against this repo's own
      `AGENTS.md` and commit the managed block.
- [x] Tests: merge into a pre-existing `AGENTS.md` with surrounding
...
- [x] Parametrize over `vscode`, `claude-code`, `cursor`, `codex`,
      `both`, `all`: build into a temp dir and assert the expected
      artifact set per target (and the absence of other targets'
      artifacts).
- [x] Assert every emitted artifact is token-free
      (`find_unresolved_tokens`) and deterministic (double-build,
      identical bytes).
- [x] Assert the `AGENTS.md` managed-block merge is idempotent and
      preserves out-of-marker content.
- [x] Assert `suppress_skill_invocability` fires only for `vscode`.

### Task Group 6: CI `editor.target` dimension
Files: `.github/workflows/ci.yml`

- [x] Extend the Sprint 3 profile build loop with an `editor.target`
...
- [x] Keep the canonical-build-command doc check (ITEM-011) intact —
      target builds are additional, not a new canonical command.
- [x] ITEM-021: run the canonical example-config build last so the
...
- [x] standard — 533 passed / 11 skipped, green with `-W error::DeprecationWarning` (2026-06-10)
- [x] Determinism — double build+deploy byte-identical incl. AGENTS.md merge; clean-clone reproducibility verified (2026-06-10)
- [x] guardrail — check_tokens.py + check_build_command.py exit 0; new dirs in post-deploy scan (2026-06-10)
- [x] docs — doc/voice subset green; PLATFORMS.md matrix pinned cell-for-cell by tests (2026-06-10)

## Success Criteria

- [x] All four platform targets emit native artifacts; `codex` is a valid `editor.target`.
- [x] `.claude/agents/`, `.cursor/commands/`, and the `AGENTS.md` managed block are seeded deterministically and idempotently.
- [x] This repo's own `AGENTS.md` carries the dogfooded managed block with no content lost outside markers.
- [x] `tests/test_platform_emission.py` passes for every target; CI builds every target token-free.
- [x] README Codex footnote removed; platform table backed by tests; `docs/PLATFORMS.md` published.
- [x] `suppress_skill_invocability` remains vscode-only.
