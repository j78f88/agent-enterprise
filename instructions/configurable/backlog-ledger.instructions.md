...
Read the ledger to find the highest existing {{ids.item_prefix}}-NNN; assign NNN+1. IDs are never reused.
...
| Condition                      | Effect                                                                 |
| ------------------------------ | ---------------------------------------------------------------------- |
| Def {{escalation.def_p0_threshold}}                        | Item escalates to P0 — mandatory inclusion in next composition         |
| Def {{escalation.def_kill_threshold}}                        | Item must be resolved or killed — no further deferral permitted        |
| Age {{escalation.age_stale_sprints}} sprints (Status: open) | Item flagged as stale — review for kill or schedule at next composition |
...
1. **ID uniqueness.** Every {{ids.item_prefix}}-NNN in the ledger is unique.
...
6. **No status in detail stores.** {{paths.bug_backlog}} and {{paths.rejections}} entries must not contain Status fields.
