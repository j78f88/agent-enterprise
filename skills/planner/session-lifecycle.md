...
2. **Read ledger summary** from `{{paths.backlog_ledger}}` — note open item counts by type, escalated items (Def ≥ {{escalation.def_p0_threshold}}), and debt pressure score. Present a one-line status: "Ledger: X open items (Y bugs, Z debt), N escalated (Def ≥ {{escalation.def_p0_threshold}}), debt pressure: P."
3. **List files in `{{paths.drafts}}`** — ignore `_`-prefixed files.
...
1. Read the ledger for items with Def ≥ {{escalation.def_p0_threshold}} (P0 mandatory).
2. If any exist, surface them via `#tool:askQuestions`: "N carry-over items have Def ≥ {{escalation.def_p0_threshold}} (mandatory P0). Include in this plan, schedule separately, or kill?"
3. Items with Def ≥ {{escalation.def_kill_threshold}}: escalate more forcefully — must be resolved or killed, no further deferral permitted.
...
When promoting a draft plan to `{{paths.sprints}}sprint-N/PLAN.md` (and the sprint is NOT part of an engagement):
...
2. Append a REJ-NNN entry to `{{paths.rejections}}`. Required fields: Severity, From, To, Raised, Status (OPEN), Context, Reason, Proposed resolution.
...
Before showing any handoff button, write a manifest to `{{paths.handoffs}}<date>-planner-to-<to>-<slug>.md`:
...
date: YYYY-MM-DD
feature: <slug>
artifact: {{paths.sprints}}sprint-N/PLAN.md
status: promoted
