...
2. **Append a REJ-NNN entry** to `docs/planning/HANDOFF_REJECTIONS.md` below the `<!-- @planner appends new entries below this line -->` marker. Full schema: `.github/instructions/handoff-rejection-format.instructions.md`. Required fields: Severity, From (@planner), To (upstream agent), Raised (date), Status (OPEN), Context (what was handed over), Reason (specific conflict — cite ADR-NNN or store count or dep), Proposed resolution (split / defer / re-approach).
...
Before showing any handoff button (including rejection loopbacks), write a manifest to `docs/planning/_handoffs/<date>-planner-to-<to>-<slug>.md`:
...
date: YYYY-MM-DD
feature: <slug>
artifact: sprints/sprint-N/PLAN.md # or REJ-NNN reference for rejections
...
2. **Read ledger summary** from `docs/planning/BACKLOG_LEDGER.md` — note open item counts by type, escalated items (Def ≥ 3), and debt pressure score. This replaces reading BUG_BACKLOG for status and HANDOFF_REJECTIONS for counts. Present a one-line status: "Ledger: X open items (Y bugs, Z debt), N escalated (Def ≥ 3), debt pressure: P."
3. **List files in `docs/planning/drafts/`** — ignore `_`-prefixed files (transient system files like `_composition-draft.md`).
...
1. **Read the ledger** for items with Def ≥ 3 (P0 mandatory per escalation rules)
2. If any exist, surface them via `#tool:askQuestions`: "**N carry-over items have Def ≥ 3** (mandatory P0 — must be included or killed). Include in this plan, schedule separately, or kill?"
...
3. Items with Def ≥ 5: escalate more forcefully — "**ITEM-NNN has been deferred 5+ times.** It must be resolved or killed — no further deferral permitted."
...
When promoting a draft plan to `sprints/sprint-N/PLAN.md` (via `/promote-draft` or manual promotion), **and the sprint is NOT part of an engagement** (delivery-lead already seeds RETRO.md for engagements):
