...
- Any chunk over 200 (flag as WARNING)
- Any chunk over 500 (flag as CRITICAL)
...
node -e "const s=Date.now(); require('child_process').execSync('pnpm build', {stdio:'inherit'}); console.log(\`Build time: \${((Date.now()-s)/1000).toFixed(1)}s\`)"
```

(On Windows/PowerShell, `Measure-Command { pnpm build 2>&1 } | Select-Object TotalSeconds` also works.)

Report build duration. Flag if over 60 as WARNING.
