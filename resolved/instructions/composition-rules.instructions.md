...
When computing P1/P2/P6 tiers for bug-type items, read the docs/planning/BUG_BACKLOG.md entry referenced in the Source column to determine the native severity classification (🔴/🟡/🟢/⚪).
...
Ties are broken by ITEM-NNN order (lower ID first).

## Composition Constraints

| Constraint          | Rule                                                                              |
| ------------------- | --------------------------------------------------------------------------------- |
| Feature cap         | Features (P3 + P4) ≤ 70% of sprint capacity                                      |
...
| Sprint size         | Target 5–8 task groups per sprint (velocity reference from SPRINTS.md)            |
| P0 mandatory        | All P0 items must be included — they cannot be deferred                           |

## P0 Overflow Relief Valve

If P0 items alone consume > 50% of sprint capacity:
...
| ---------------------- | ----- | -------------------------------------------------------------------- |
| `DEBT_WARNING_SPRINTS` | 3     | Warn if any debt item has Def 3 (also triggers P0 escalation)     |
| `DEBT_ESCALATE_SPRINTS`| 5     | Escalate: must resolve or kill (no further deferral)                 |
| `DEBT_WARNING_ITEMS`   | 20    | Warn if debtPressure 20                                            |
| `DEBT_ESCALATE_ITEMS`  | 40    | Escalate: debtPressure 40 — mandatory debt sprint before any new features |
| `DEBT_MIN_ALLOCATION`  | 30    | When debt pressure advisory active, 30% capacity allocated to debt |
...
When `debtPressure` exceeds `DEBT_WARNING_ITEMS` (20), include a debt health warning in the composition output. When it exceeds `DEBT_ESCALATE_ITEMS` (40), block feature-only sprints.
