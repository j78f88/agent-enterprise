...
- `a11y` is recommended-default when the sprint touches `apps/web/src/components/**`.
...
3. Run store test suite: `pnpm test` — all migration tests must pass.
...
1. **Prepend** a new `ChangelogEntry` to `docs/user/changelog.json` with the next incremented version, today's date, and a user-friendly title + highlights.
2. **Copy** the updated file to `apps/web/public/changelog.json` (the web app fetches from `public/`).
3. **Bump** the `version` field in `apps/web/package.json` to match the new entry. This version drives the `UpdateNotification` banner — a mismatch causes the notification to either never show or show on every reload.
4. **Verify** JSON validity: `Get-Content docs/user/changelog.json -Raw | ConvertFrom-Json` must succeed.
