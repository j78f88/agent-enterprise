...
- Conflicts with docs/NON_GOALS.md, docs/planning/ROADMAP.md, docs/architecture/DECISIONS.md
- Risk assessment
...
**Trigger:** After merge to master and production deployment workflow finishes.
...
- docs/architecture/DECISIONS.md — check for ADR conflicts
...
Run pre-flight checks (docs/development/TECHNICAL_DEBT.md, docs/architecture/DECISIONS.md, docs/architecture/FUTURE_CONSIDERATIONS.md, docs/NON_GOALS.md).
...
- **Test deploy workflow:** `deploy-test.yml`
- **Production deploy workflow:** `deploy-azure-swa.yml` (Azure Static Web Apps CI/CD)
...
- **Deploy check commands:**
  ```bash
  # Test deployment
  gh run list --branch develop --workflow deploy-test.yml --limit 3 --json databaseId,status,conclusion
  # Production deployment
  gh run list --branch master --workflow "Azure Static Web Apps CI/CD" --limit 2 --json databaseId,status,conclusion
  # Failure details
