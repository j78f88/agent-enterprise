...
2. **The cost-governance hook is already owned by core (as planned scope), not Cluster 8.** DG-4 (least-capability manifests) already *declares* **`maxUsd` / `timeoutMs` / kill-switch as first-class declared ceilings**, against threat **T8 (runaway cost / unbounded autonomy)**, in the design-goals doc's planned capability-manifest scope (its "Sprint 4" section). This is **planned, not shipped**: the actually-executed Sprint 4 was Platform Parity and Sprint 5 was the Mode 2 Dispatcher, and there is no `maxUsd`/`timeoutMs` code in `src/` today. The only Cluster-8 idea that the project's *core mission* actually wants is "an agent's cost must be bounded by a declared ceiling that the build compiles into a real gate." That is a **compliance control**, not an efficiency feature — it limits blast radius, it does not optimise spend.
...
Net: validate Cluster 8 as **template-referenced controls**, with the single load-bearing exception that the *declared cost ceiling* (planned DG-4/T8 core scope, not yet shipped) is the one place a Cluster-8 idea becomes a real compiled gate.
...
| 5 | Complexity cost | **FAIL (build the ladder); PASS (compile a declared field)** | Building or operating a model router/ladder is a runtime system we have disowned — enormous cost, zero fit. Adding a declared `model_tier:` / assumed-rung field that DG-4's ceiling references is near-free and rides the *already-planned* DG-4 capability-manifest scope (design-goals §"Sprint 4", `maxUsd`/`timeoutMs`/kill-switch). Note: that scope is **planned, not shipped** — the actually-shipped Sprint 4 was Platform Parity and Sprint 5 was the Mode 2 Dispatcher; no `maxUsd` code exists in `src/` yet. |
...
| 5 | Complexity cost | **PASS (declared ceiling); FAIL (graceful-degradation engine)** | The declared-ceiling field is already in the *planned* DG-4 capability-manifest scope (design-goals §"Sprint 4") — incremental, in-boundary, low cost — though planned, not yet shipped (no `maxUsd` code in `src/`; real Sprints 4/5 were Platform Parity and the Mode 2 Dispatcher). The research's "soft cap → downshift / compress / checkpoint-resume" unmet need is a *runtime* engine and is out of bounds; do not build it. |
...
- **Reframed:** *Scope down to the declared cost/time ceiling only* — which is **already planned core scope (DG-4, threat T8):** `maxUsd` / `timeoutMs` / kill-switch as first-class declared manifest fields the resolver compiles into a real gate. **Planned, not shipped** — this is in the design-goals doc's capability-manifest scope (its "Sprint 4" section), but it has not been built (no `maxUsd` code in `src/`; the executed Sprint 4 was Platform Parity, Sprint 5 was the Mode 2 Dispatcher). Drop the reasoning-effort dials, the four-quadrant frame, and the soft-cap engine as runtime/out-of-boundary. The AU-gov template references "every autonomous agent must carry a declared spend/time ceiling" as a compliance control.
- **Why the reframe rescues it:** The full pattern failed Tests 1/2/5 on its runtime-dial and degradation-engine parts. Restricting to the **declared ceiling** sub-pattern passes all five and — crucially — does not invent new work: it confirms and reinforces an existing core threat (T8) and an already-planned DG-4 design goal, rather than proposing fresh scope. This is the one place a Cluster-8 idea becomes a genuine compiled gate rather than a referenced adopter concern. (Maturity note for `@planner`: the ceiling is in-scope per design but unbuilt — it is real planning work, not a no-op documentation pass.)
...
| 4 — Token-spend budgeting | REFRAMED | Scoped down to the declared cost/time ceiling — already planned core scope (DG-4 / T8), though not yet shipped; runtime dials and soft-cap engine dropped. |
...
**Headline:** Only **one** Cluster-8 idea survives as buildable, and it is not new scope — the declared **cost/time ceiling** is already part of the *planned* DG-4 capability-manifest design (threat T8). It is **planned, not shipped**: no `maxUsd`/`timeoutMs` exists in `src/` today (the executed Sprints 4/5 were Platform Parity and the Mode 2 Dispatcher). The remaining four levers are runtime cost optimisations that fall outside agent-enterprise's build-time boundary; they belong in the AU-gov compliance template as *adopter controls to reference*, not substrate to build. This is the expected result for an ADJACENT efficiency cluster held to the strict Causation and Complexity-cost bar.
